/* ================================================================
   ...
   script.js — Supabase connected (bigint IDs)
   ================================================================ */
'use strict';
...
function nextAuditID(nursery) {
  const n = records.filter(r=>r.nursery===nursery).length;
...
function setLoading(on) {
  const o=document.getElementById('loading-overlay'); if(o) o.classList.toggle('hidden',!on);
}
...
function handlePhoto(input){
  ...
  reader.onload=e=>{ formState.photo=e.target.result; document.getElementById('f-photo-img').src=e.target.result; document.getElementById('photo-drop').style.display='none'; document.getElementById('photo-preview').style.display='block'; };
...
async function saveRecord(){
  ...
  try {
    ...
    if(photoUrl&&photoUrl.startsWith('data:')) photoUrl=await sb.uploadPhoto('audit-photos',`plot_${plot}`,photoUrl);
    const payload={ nursery:formState.nursery, plot, batch:batch||null, pest:formState.ulat, tikus:formState.tikus, disease:formState.bintik, warna_daun:formState.warna, photo_url:photoUrl||null, date:todayISO() };
    if(editMode&&editId){ await sb.update('plot_audits',editId,payload); showToast('✓ Record updated'); }
    else { payload.audit_id=nextAuditID(formState.nursery); await sb.insert('plot_audits',payload); showToast('✓ Record saved'); }
    await loadRecords(); setView('list');
  } catch(e){ showToast('⚠ Save failed — check console'); console.error(e); setLoading(false); }
}

...
function nextRecordID(nursery) {
  const existing = records.filter(r => r.nursery === nursery);
  return `HGT-${nursery}-${pad(existing.length + 1)}`;
}
...
async function loadRecords() {
  ...
  try {
    ...
    records = rows.map(r => ({
      uid: r.id, id: r.record_id,
      nursery: r.nursery, plot: r.plot, batch: r.batch,
...
function renderList() {
  ...
  document.querySelectorAll('.tab-item').forEach(t => {
    const n = t.dataset.n; const cnt = records.filter(r => r.nursery===n).length;
  ...
  if (!recs.length) {
    listEl.innerHTML = `<div class="empty-state"><div class="empty-state-icon"><svg viewBox="0 0 24 24"><line x1="12" y1="2" x2="12" y2="22"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg></div><h3>No height records yet</h3><p>Tap <strong>+</strong> to record the first seedling heights for ${NURSERY_LABELS[activeTab]}.</p></div>`;
    return;
  }
  ...
  listEl.innerHTML = recs.map(r => {
    ...
    const thumbs = [r.p1,r.p2,r.p3].map(p =>
      p ? `<img src="${p}" alt="photo" />`
        : `<div class="thumb-placeholder"><svg viewBox="0 0 24 24"><rect x="3" y="5" width="18" height="14" rx="2"/><circle cx="12" cy="12" r="3"/></svg></div>`
    ).join('');
    return `
...
function openEdit(uid) {
  ...
  formState = { nursery:rec.nursery, s1:rec.s1,s2:rec.s2,s3:rec.s3, p1:rec.p1,p2:rec.p2,p3:rec.p3 };
  populateForm(rec); setView('form');
...
function populateForm(rec) {
  ...
  (NURSERY_PLOTS[formState.nursery]||[]).forEach(p => {
    ...
    if (rec && rec.plot===p) o.selected=true; plotSel.appendChild(o);
  });
...
function onHeightInput(sample, el) {
  formState[`s${sample}`] = el.value.trim();
  updateAvgDisplay();
  const fb = document.getElementById(`s${sample}-fb`);
  if (fb) fb.textContent = el.value && !isNaN(parseFloat(el.value)) && parseFloat(el.value)>0 ? '✓' : '';
}
...
function renderPhotoSlot(n, src) {
  ...
  if (src) {
    ...
    const clr=document.createElement('button'); clr.className='photo-slot-clear'; clr.innerHTML='×';
    clr.onclick=e=>{e.stopPropagation();clearPhotoSlot(n);}; slot.appendChild(clr);
  } else {
    ...
    icon.innerHTML='<rect x="3" y="5" width="18" height="14" rx="2"/><circle cx="12" cy="12" r="3"/><path d="M9 5l1.5-2h3L15 5"/>';
...
function handlePhotoUpload(n, input) {
  ...
  reader.onload = e => {
    formState[`p${n}`]=e.target.result; renderPhotoSlot(n,e.target.result);
    const note=document.getElementById('photo-req-note');
    if (formState.p1&&formState.p2&&formState.p3&&note) { note.classList.remove('error'); note.textContent='3 photos required (one per sample)'; }
  };
...
async function saveRecord() {
  ...
  try {
    ...
    async function maybeUpload(photo, label) {
      if (photo && photo.startsWith('data:')) return await sb.uploadPhoto('audit-photos', label, photo);
      return photo;
    }
    const [p1url, p2url, p3url] = await Promise.all([
      maybeUpload(formState.p1, `height_${plot}_s1`),
      maybeUpload(formState.p2, `height_${plot}_s2`),
      maybeUpload(formState.p3, `height_${plot}_s3`)
    ]);
    ...
    const payload = {
      ...
      photo_1_url: p1url||null, photo_2_url: p2url||null, photo_3_url: p3url||null,
      date: todayISO()
    };

    if (editMode && editId) {
      await sb.update('height_records', editId, payload);
      showToast('✓ Record updated');
    } else {
      payload.record_id = nextRecordID(formState.nursery);
      await sb.insert('height_records', payload);
...
function openDetail(uid) {
  ...
  detailId = uid;
  [1,2,3].forEach(n => {
    const el = document.getElementById(`detail-p${n}`); if (!el) return;
    el.innerHTML = '';
    if (rec[`p${n}`]) {
      const img=document.createElement('img'); img.src=rec[`p${n}`]; img.alt=`S${n}`;
      img.onclick=()=>openLightbox(rec[`p${n}`]); el.appendChild(img);

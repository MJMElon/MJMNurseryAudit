...
async function loadAll(){
  ...
  try{
    const [tRows, aRows] = await Promise.all([
      sb.select('maintenance_tasks','select=*'),
      sb.select('maintenance_audits','select=*')
    ]);
...
async function saveAudit(){
  ...
  try{
    ...
    const result=await smartSave('maintenance_audits',editMode?'update':'insert',
...
async function doDelete(){
  ...
  setLoading(true);
  try{
    await sb.delete('maintenance_audits',deleteTarget);deleteTarget=null;
